...
const Orders = () => {
    ...
    const fetchOrders = async () => {
      try {
        const response = await axios.get(
          `${API_BASE_URL}/api/orders/getall`,
          {
            headers: {
  ...
  const handleStatusChange = async (orderId, newStatus) => {
    try {
      await axios.put(
        `${API_BASE_URL}/api/orders/getall/${orderId}`,
        { status: newStatus },
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('authToken')}`,
                ...
                <tbody>
                  {orders.map(order => {
                    ...
                    const payStatusStyle =
                      statusStyles[order.paymentStatus] || statusStyles.processing;
                                ...
                                <img
                                  src={
                                    itm.item.imageUrl?.startsWith('http')
                                      ? itm.item.imageUrl
                                      : `${API_BASE_URL}${itm.item.imageUrl}`
                                  }
                                  alt={itm.item.name}
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            >
                              {Object.entries(statusStyles)
                                ...
                                .map(([key, sty]) => (
                                  <option key={key} value={key} className={`${sty.bg} ${sty.color}`}>
                                    {sty.label}
                                  </option>
                                ))}
                            </select>
